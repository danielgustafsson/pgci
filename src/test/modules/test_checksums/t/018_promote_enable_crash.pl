...
$ctl = `pg_controldata -D @{[ $standby->data_dir ]}`;
...
note("standby control file after the promotion: $after, checkpoint $ckpt, redo $redo");
...
ok($started, 'promoted node restarts after crashing before its first checkpoint');
