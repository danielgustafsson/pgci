...
$node->init(no_data_checksums => 1, has_archiving => 1, allows_streaming => 1);
...
# Full backup while checksums are off.
$node->backup('full');

# Offline enable, which rewrites every page but logs nothing.
$node->stop;
system_or_bail('pg_checksums', '--enable', '--pgdata', $node->data_dir);
$node->start;
test_checksum_state($node, 'on');

# A little activity, then an incremental backup while checksums are on.
$node->safe_psql('postgres',
...
# The other direction is harmless.  Take a full backup with checksums on,
# disable them offline, and continue with an incremental: the last backup
# wins, the combined cluster comes up with checksums off and the stale
# checksums inherited from the full backup are never verified.
$node->backup('full2');
...
$restored->init_from_backup($node, 'incr2',
