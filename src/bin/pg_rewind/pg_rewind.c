...
sanityChecks(void)
{
	 ...
	 * enabled them online), and no page can fail verification.  That does not
	 * hold when the target is a standby, whose checkpoints were written by
	 * its upstream primary; the divergence checkpoint is checked against both
	 * nodes once it has been located.  Warn here so that an offline change on
