...
ProcessGetMemoryContextInterrupt(void)
{
	...
	foreach_ptr(MemoryContextData, cur, contexts)
	{
		...
		if (context_id <= (max_stats - 2) || stats_count <= max_stats)
		{
		 ...
		 *
		 * We can store contexts from 0 to max_stats - 1. When stats_count
		 * is greater than max_stats, we stop reporting individual statistics
					 ...
					 dsa_area *area, int max_levels)
{
	...
	else
	{
		...
		memctx_info[curr_id].levels = levels;
