...
static const char *
pgstat_get_wait_buffer(WaitEventBuffer w)
{
	const char *event_name = "unknown wait event";

	switch (w)
	{
		 case WAIT_EVENT_BUFFER_CLEANUP:
			 event_name = "BufferCleanup";
			 break;
		 case WAIT_EVENT_BUFFER_EXCLUSIVE:
			 event_name = "BufferExclusive";
			 break;
		 case WAIT_EVENT_BUFFER_SHARED:
			 event_name = "BufferShared";
			 break;
		 case WAIT_EVENT_BUFFER_SHARE_EXCLUSIVE:
			 event_name = "BufferShareExclusive";
			 break;
			 /* no default case, so that compiler will warn */
	}
...
pgstat_get_wait_io(WaitEventIO w)
{
	...
	switch (w)
	{
		 ...
		 case WAIT_EVENT_COPY_FROM_READ:
			 event_name = "CopyFromRead";
			 break;
		 case WAIT_EVENT_COPY_TO_WRITE:
			 event_name = "CopyToWrite";
			 break;
...
pgstat_get_wait_ipc(WaitEventIPC w)
{
	...
	switch (w)
	{
		 ...
		 case WAIT_EVENT_CHECKSUM_ENABLE_TEMPTABLE_WAIT:
			 event_name = "ChecksumEnableTemptableWait";
			 break;
