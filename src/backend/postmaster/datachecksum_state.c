...
enable_data_checksums(PG_FUNCTION_ARGS)
{
	 ...
	 * A database which turns invalid after this check is handled by the
...
ProcessDatabase(DataChecksumsWorkerDatabase *db)
{
	 ...
	 * invalidated by a DROP DATABASE which is bound to remove its files, after
	 * we built the database list then that is the expected outcome and not an
