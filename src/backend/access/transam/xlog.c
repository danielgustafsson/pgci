...
UpdateControlFile(void)
{
	...
	SpinLockAcquire(&XLogCtl->info_lck);
	a = XLogCtl->data_checksum_version;
	/*
	 * XXX: During shutdown this gets called with data_checksum_version in the
	 * ControlFile set to zero. Below is an ugly hack to fix till bug is found.
	 */
	if (a != ControlFile->data_checksum_version)
		ControlFile->data_checksum_version = a;
	SpinLockRelease(&XLogCtl->info_lck);
...
StartupXLOG(void)
{
	...
	/*
	 * If data checksums were enabled as an offline operation while the cluster
	 * was stopped, transition the state of on.
	 */
...
xlog2_redo(XLogReaderState *record)
{
	...
	if (info == XLOG2_CHECKSUMS)
	{
		...
		SpinLockAcquire(&XLogCtl->info_lck);
		old = XLogCtl->data_checksum_version;
