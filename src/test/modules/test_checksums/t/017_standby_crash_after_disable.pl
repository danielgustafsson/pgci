...
my $ctl_before = `pg_controldata -D @{[ $standby->data_dir ]}`;
my ($ctl_state) = $ctl_before =~ /Data page checksum version:\s+(\d+)/;
print "# standby control file data_checksum_version after the disable: "
...
ok($started, 'standby restarts after crashing between a checksum state '
