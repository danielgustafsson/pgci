			...
			int options, BulkInsertState bistate)
{
	...
	if (PageIsAllVisible(BufferGetPage(buffer)))
	{
		...
		visibilitymap_clear_vmbits(relation,
...
l1:
	...
	if (PageIsAllVisible(page))
	{
		all_visible_cleared = true;
		PageClearAllVisible(page);
		visibilitymap_clear_vmbits(relation, BufferGetBlockNumber(buffer),
							vmbuffer, VISIBILITYMAP_VALID_BITS);
	}
...
l2:
	...
	if (need_toast || newtupsize > pagefree)
	{
		 ...
		 */
		if (PageIsAllVisible(page) &&
			visibilitymap_clear_vmbits(relation, block, vmbuffer,
								VISIBILITYMAP_ALL_FROZEN))
	...
	if (newbuf != buffer && PageIsAllVisible(BufferGetPage(newbuf)))
	{
		...
		visibilitymap_clear_vmbits(relation, BufferGetBlockNumber(newbuf),
...
l4:
		...
		if (PageIsAllVisible(BufferGetPage(buf)) &&
			visibilitymap_clear(rel, block, vmbuffer, InvalidXLogRecPtr, xid,
								VISIBILITYMAP_ALL_FROZEN))
			cleared_all_frozen = true;
